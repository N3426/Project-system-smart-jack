...
const int trigPin = 7; // Adjust pin number as needed
const int echoPin = 8; // Adjust pin number as needed
...
const int minDistance = 7; // Minimum distance in cm
const int maxDistance = 31; // Maximum distance in cm
  ...
  if (Serial.available() > 0) {
    ...
    if (Incoming_value == '1') {
        if (distance > minDistance && distance < maxDistance) {
            ...
            Serial.println("Object out of range, stopping motor");
            analogWrite(LPWM, 0);
            analogWrite(RPWM, 0);
        }
    }
    // Control logic for '0'
        ...
        if (distance > minDistance && distance < maxDistance) {
            Serial.println("Object within range, motor moving anticlockwise");
